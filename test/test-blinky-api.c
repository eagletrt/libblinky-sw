...
struct Blinky bOnce;
struct Blinky bRepeat;

void setUp(void) {
    blinky_api_init(&bOnce, pattern, PATTERN_SIZE, false, BLINKY_HIGH);
    blinky_api_init(&bRepeat, pattern, PATTERN_SIZE, true, BLINKY_HIGH);
}
...
void check_blink_init_null(void) {
    struct Blinky *b = NULL;
    blinky_api_init(b, NULL, 0, false, BLINKY_HIGH);
    TEST_ASSERT_NULL(b);
}
void check_blink_enable_null(void) {
    struct Blinky *b = NULL;
    blinky_api_enable(b, false);
    TEST_ASSERT_NULL(b);
}
void check_blink_repeat_null(void) {
    struct Blinky *b = NULL;
    blinky_api_repeat(b, true);
    TEST_ASSERT_NULL(b);
}
void check_blink_set_pattern_null(void) {
    struct Blinky *b = NULL;
    blinky_api_set_pattern(b, NULL, 0);
    TEST_ASSERT_NULL(b);
}
void check_blink_reset_null(void) {
    struct Blinky *b = NULL;
    blinky_api_reset(b, BLINKY_HIGH);
    TEST_ASSERT_NULL(b);
}
void check_blink_routine_null(void) {
    struct Blinky *b = NULL;
    blinky_api_routine(b, 300);
    TEST_ASSERT_NULL(b);
}
...
void check_blink_repeated(void) {
    blinky_api_repeat(&bOnce, true);
    TEST_ASSERT_TRUE(bOnce.repeat);
}
void check_blink_once(void) {
    blinky_api_repeat(&bOnce, false);
...
void check_blink_set_pattern_size(void) {
    blinky_api_set_pattern(&bOnce, otherPattern, OTHER_PATTERN_SIZE);
...
void check_blink_reset_time(void) {
    blinky_api_reset(&bOnce, BLINKY_LOW);
...
void check_blink_routine_with_null_handler(void) {
    TEST_ASSERT_EQUAL_INT(BLINKY_LOW, blinky_api_routine(NULL, 0));
}
void check_blink_routine_with_null_pattern(void) {
    bOnce.pattern = NULL;
    TEST_ASSERT_EQUAL_INT(BLINKY_LOW, blinky_api_routine(&bOnce, 0));
}
...
void check_blink_routine_start_high(void) {
    TEST_ASSERT_EQUAL_INT(BLINKY_HIGH, blinky_api_routine(&bOnce, 0));
}
void check_blink_routine_change_state(void) {
    TEST_ASSERT_EQUAL_INT(BLINKY_HIGH, blinky_api_routine(&bOnce, 500));
    TEST_ASSERT_EQUAL_INT(BLINKY_LOW, blinky_api_routine(&bOnce, 1500));
}
void check_blink_routine_runned_once(void) {
    bOnce.index = 2;
    blinky_api_routine(&bOnce, 999);
    TEST_ASSERT_TRUE(bOnce.enable);
    blinky_api_routine(&bOnce, 1001);
    TEST_ASSERT_FALSE(bOnce.enable);
}
void check_blink_routine_repeated_state(void) {
    bRepeat.index = 2;
    TEST_ASSERT_EQUAL_INT(BLINKY_HIGH, blinky_api_routine(&bRepeat, 999));
    TEST_ASSERT_EQUAL_INT(bRepeat.initial_state, blinky_api_routine(&bRepeat, 1001));
}
void check_blink_routine_repeated_enable(void) {
    bRepeat.index = 2;
    blinky_api_routine(&bRepeat, 1001);
